...
def every_4(mylist):
    ...
    for index in range(0, len(mylist)-1, 4):
        opp_code = mylist[index]
        
        if opp_code == 99:
            break
        else:
            first_read_location, second_read_location, write_location = mylist[index + 1: index + 4]
        
        if opp_code == 1:
            mylist[write_location] = mylist[first_read_location] + mylist[second_read_location]
...
# also just like before. Each time you try a pair of inputs, make sure you first reset 
...
def return_noun_and_verb():

    for noun in range(0, 100):
        initial_list = day2list.input_list
        for verb in range(0,100):
            test_list = initial_list[0:]
            test_list[1], test_list[2] = noun, verb
            output = every_4(test_list)
            if output == 19690720:
                return 100*noun + verb

return_noun_and_verb()
