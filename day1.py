...
def get_mass_list(csv_file):
    mass_list=[]
    with open(csv_file, newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
        for row in reader:
            mass_list.append(int(row[0]))
    return mass_list

def fuel_for_module(masses):
    return sum([(mass//3)-2 for mass in masses])

def fuel_for_module_list(masses):
    return [(mass//3)-2 for mass in masses]

print('Day 1 Part 1')
print(fuel_for_module(get_mass_list('day1csv1.csv')))
...
def fuel_for_fuel(fuel):
    fuel_required = fuel
    while fuel > 0:
        additional_fuel = fuel_for_module([fuel])
        fuel = additional_fuel
        if fuel > 0:
            fuel_required += additional_fuel
    
    return fuel_required

def total_fuel_fuel(fuel_list):
    return sum([fuel_for_fuel(fuel) for fuel in fuel_list])



print('Day 1 Part 2')
print(total_fuel_fuel(fuel_for_module_list(get_mass_list('day1csv1.csv'))))
